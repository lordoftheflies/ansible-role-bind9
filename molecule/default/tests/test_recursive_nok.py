...
testinfra_hosts = testinfra_ansible_runner.get_hosts('ansible-test-outside')

ns1_facts = testinfra_ansible_runner.run_module('ansible-test-ns1', 'setup', None)['ansible_facts']


def test_recursive_nok_ip4(host):
    r = host.check_output("dig -4 www.redhat.com. @%s" % ns1_facts['ansible_default_ipv4']['address'])
    assert "WARNING: recursion requested but not available" in r

def test_recursive_nok_ip6(host):
